...
def country_distribution(games_df: DataFrame) -> DataFrame:
...
def win_percentage_by_time_class(games_df: DataFrame) -> DataFrame:
    return games_df.groupby(games_df.time_class).agg(F.count(games_df.time_class).alias("times_played"),
                                                     F.avg(games_df.won.cast("double")).alias("win_percentage"))
